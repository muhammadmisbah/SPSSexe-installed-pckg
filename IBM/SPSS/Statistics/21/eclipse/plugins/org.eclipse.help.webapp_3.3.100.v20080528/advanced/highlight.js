		...
		while(index>=0){
			...
			if(isSafari){
				...
				if ((defaultHighlight == false) && !document.styleSheets) {
					...
					spanNode.className=null;
				}
				else {
					//if (isIE) spanNode.setAttribute("className","resultofText"); --- fit for IE7, but not for IE8
					//else spanNode.setAttribute("class","resultofText"); --- fit for FF/IE8, but not for IE7
					spanNode.className='resultofText';
					//Fixed on 10/15/09 by Chang Bo XIAO
				}
			}
